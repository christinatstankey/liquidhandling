...
Usage:
    ...
    python tools/build_handling_profiles.py --min-rules 1  # report records with >= 1 rule fired
"""
...
PROFILE_SCHEMA_PATH = REPO_ROOT / "data" / "schemas" / "handling-profile-v1.0.json"

# Import apply_rules from the ingest package directly
sys.path.insert(0, str(REPO_ROOT / "ingest"))
from apply_rules import apply_rules, validate_rules  # noqa: E402


def main():
    ap = argparse.ArgumentParser(description=__doc__.split("\n")[1])
    ap.add_argument("--dry-run",   action="store_true", help="Print stats without writing files.")
    ap.add_argument("--min-rules", type=int, default=None,
                    help="Print per-record output for records with at least this many rules fired. Use 0 to print all.")
    args = ap.parse_args()
    ...
    for jf in paths:
        ...
        if args.min_rules is not None and n >= args.min_rules:
