...
def load_lookup() -> tuple[dict[str, dict], dict[str, dict]]:
    """Return lookup tables keyed by CAS string and no-CAS reagent slug."""
    entries = yaml.safe_load(LOOKUP_PATH.read_text())
    by_cas = {e["cas"]: e for e in entries if e.get("cas") and e.get("cas") != "N/A"}
    by_slug = {e["slug"]: e for e in entries if e.get("slug")}
    return by_cas, by_slug


def enrich(reagent: dict, lookup: tuple[dict[str, dict], dict[str, dict]], slug: str | None = None) -> tuple[dict, list[str]]:
    by_cas, by_slug = lookup
    cas     = reagent.get("cas")
    props   = reagent.get("properties", {})
    changes: list[str] = []

    entry = by_cas.get(cas) if cas else by_slug.get(slug or "")
    if entry is None:
        identifier = cas if cas else (slug or "no-CAS")
        return reagent, [f"  ({identifier!r} not in chebi-lookup.yaml)"]
...
def process(json_path: Path, lookup: tuple[dict[str, dict], dict[str, dict]], dry_run: bool = False) -> None:
    ...
    updated, changes = enrich(reagent, lookup, slug=json_path.stem)
