...
HCODE_FLAG_MAP: dict[str, list[tuple[str, bool]]] = {
    ...
    # Acute toxicity signals: not every acute-toxic reagent is volatile, but
    # powder/aerosol handling and automated transfer should happen with primary
    # containment. Map to fume_hood_required conservatively rather than adding
    # a new schema flag midstream.
    "H300": [("fume_hood_required", True)],
    "H301": [("fume_hood_required", True)],
    "H310": [("fume_hood_required", True)],
    "H330": [("fume_hood_required", True)],
...
CONDITIONAL_HCODE_FLAG_MAP: dict[str, list[tuple[str, bool, str]]] = {
    # H335 is respiratory irritation. Require SDS Section 8 engineering or
    # respiratory-control language before escalating to hood handling.
    "H335": [("fume_hood_required", True, "section_8_ventilation")],
}

SECTION8_VENTILATION_RE = re.compile(
    r"\b(local exhaust|fume hood|adequate ventilation|mechanical ventilation|"
    r"respiratory protection|suitable respirator)\b",
    re.IGNORECASE,
)
...
def _has_sds_source(props: dict) -> bool:
    return any(
        source.get("type") in SDS_SOURCE_TYPES
...
def _set_sds_metadata(
    reagent: dict,
    sections: dict[int, str],
    h_codes: set[str],
    actionable: bool,
) -> bool:
    """Record that the SDS was parsed even when no handling flag was found."""
    sds_source = reagent.setdefault("sds_source", {})
    unmapped_hcodes = sorted(
        h for h in h_codes
        if h not in HCODE_FLAG_MAP and h not in CONDITIONAL_HCODE_FLAG_MAP
    )
    updates = {
        "parsed_sections": sorted(sections.keys()),
        "enrichment_status": (
            "parsed_with_actionable_flags" if actionable
            else "parsed_no_actionable_flags"
        ),
        "unmapped_hcodes": unmapped_hcodes,
    }

    changed = False
    for key, value in updates.items():
        if sds_source.get(key) != value:
            sds_source[key] = value
            changed = True
    return changed
...
def enrich(reagent: dict, pdf_path: Path) -> tuple[dict, list[str]]:
    ...
    for hcode, flag_pairs in HCODE_FLAG_MAP.items():
        ...
        for flag, agrees in flag_pairs:
            ...
            if added:
                # If this SDS source confirms True, set the value. This also
                # repairs stale parser/placeholder false values from earlier
                # passes so the value matches the evidence chain.
                if props[flag].get("value") is not True and agrees:
                    props[flag]["value"] = True
                changes.append(f"  + {flag}: ghs_hcode:{hcode}")

    # --- 3b. Conditional H-code sources --------------------------------------
    section_condition_met = {
        "section_8_ventilation": bool(SECTION8_VENTILATION_RE.search(sections.get(8, ""))),
    }
    for hcode, flag_pairs in CONDITIONAL_HCODE_FLAG_MAP.items():
        if hcode not in h_codes:
            continue
        for flag, agrees, condition in flag_pairs:
            if not section_condition_met.get(condition):
                continue
            if flag not in props:
                props[flag] = _ensure_sourced(None, True if agrees else None)
            else:
                props[flag] = _ensure_sourced(props[flag], props[flag] if not isinstance(props[flag], dict) else props[flag].get("value"))
            ref = f"{hcode}+{condition}"
            added = _add_source(props[flag], "ghs_hcode", ref, agrees)
            if added:
                if props[flag].get("value") is not True and agrees:
                    props[flag]["value"] = True
                changes.append(f"  + {flag}: ghs_hcode:{ref}")
    ...
    for flag, pattern, allowed_secs, src_type in PROSE_PATTERNS:
        ...
        for sec_num, sec_text in targets:
            ...
            if added:
                if props[flag].get("value") is not True:
                    props[flag]["value"] = True
                changes.append(f"  + {flag}: {src_type}:S{sec_num}:{snippet[:30]!r}")
            break  # one hit per flag per section set is enough

    metadata_changed = _set_sds_metadata(reagent, sections, h_codes, _has_sds_source(props))
    if metadata_changed:
        changes.append("  + sds_source: parsed_sections/enrichment_status")
