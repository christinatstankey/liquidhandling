...
def _check_sourced_flags(record: dict) -> list[str]:
    ...
    for key, val in props.items():
        ...
        has_positive_evidence = any(
            source.get("agrees") is True
            and source.get("type") in POSITIVE_EVIDENCE_TYPES
            for source in val.get("sources", [])
        )
        if has_positive_evidence and val.get("value") is not True:
            warnings.append(
                f"  VALUE {key}: value={val.get('value')!r} but has positive evidence"
            )
