...
- Foamy/detergent (SDS, BSA, Tween-containing): slow dispense along tube wall.
  [rules: `detergent_no_foam` for true surfactants;
   `foam_prone_no_air_entrainment` for non-detergent foamers — concentrated
   BSA, sera, recombinant cytokine carriers, lipid suspensions/LNPs —
   keyed off the `is_foaming` flag]
...
The first four bullets are codified in `rules.yaml`: each fires when the
relevant material appears in the reagent's `plastic_incompatible` list.
Glass-attacking reagents (HF) are uncodified — see the note in `rules.yaml`
under the labware section.
...
- Polystyrene (most cell-culture plates, clear conicals) is dissolved by
  ...
  leaches styrene oligomers into the sample. [rule: `avoid_polystyrene`]
- Polypropylene (Eppendorfs, most "chemical-resistant" tubes) tolerates
  ...
  [rule: `avoid_polypropylene`]
- Polycarbonate centrifuge bottles and microfluidic chips are attacked by
  strong bases, hot alcohols, and polar aprotics (DMSO, DMF, NMP); high
  RCF after solvent exposure → stress-crack failure.
  [rule: `avoid_polycarbonate`]
- DMSO leaches plasticizers (phthalates, oligomers) from many plastics
  ...
  [encoded by listing "polypropylene" in DMSO's `plastic_incompatible`;
   long-storage caveat lives in the `avoid_polypropylene` rule body]
...
- Concentrated H₂SO₄, HNO₃, HF: never PP. Glass for the first two; PP/PTFE
  *only* for HF (HF eats glass). [glass-attack case is uncodified — see
   the note in `rules.yaml`]
- Silicone tubing absorbs hydrophobic small molecules — dosing through
  silicone lines loses drug to the wall. [rule: `avoid_silicone_tubing`]
- Parafilm dissolves in most organics; don't seal solvent-containing tubes.
  [rule: `avoid_parafilm_seal`]
...
- Fine powders can also fail mechanically before they fail chemically:
  static on plastic weigh boats, powder on caps, and airborne dust all
  create mass-transfer errors. The generic powder rule covers static/dust
  control; toxic-powder containment should stay tied to SDS hazard evidence
  or manufacturer handling guidance.
...
- Ethers (Et₂O, THF, dioxane, diisopropyl ether) form explosive peroxides
  ...
  strict "test before use / short working life" rules should be limited to
  volatile peroxide-formers unless a peroxide-former class is available]
...
Baseline PPE, SDS review, and prompt return to storage are global lab practice,
not reagent-specific handling intelligence. Keep them visible in the UI or
documentation, but don't emit them as per-reagent rules unless a specific
hazard, property, or manufacturer instruction raises the requirement.
